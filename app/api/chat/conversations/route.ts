...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedConversations = (conversations || []).map((conv) => {
      ...
      return {
        ...conv,
        other_participant: Array.isArray(otherParticipant)
          ? otherParticipant[0]
          : otherParticipant,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: existingConv } = await supabaseServer
      ...
      .select("id")
      .or(
        `and(participant1_id.eq.${user.id},participant2_id.eq.${participant_id}),and(participant1_id.eq.${participant_id},participant2_id.eq.${user.id})`
      )
      .single();
